...
export default function ProfileScreen({ onTabChange }: Props) {
  const [profile, setProfile] = useState<Profile>({ name: 'Guest', phone: '', area: 'BTM Layout', created_at: '' });
    ...
    <SafeAreaView style={s.safe}>
      ...
      <View style={s.header}>
        ...
        <View style={s.profileInfo}>
          ...
          <Text style={s.profilePhone}>{profile.phone || '+91 XXXXXXXXXX'}</Text>
      ...
      {loading ? (
        ...
        <ScrollView style={s.scroll} showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 80 }}>
          ...
          <View style={s.statsRow}>
            {[
              { val: String(totalOrders), lbl: 'Total Orders' },
              { val: '₹50',              lbl: 'Per Can'      },
              { val: profile.area || 'BTM', lbl: 'Zone'     },
            ].map((st, i) => (
          ...
          <View style={s.sec}>
            ...
            <View style={[s.addrCard, s.addrCardActive]}>
              <Text style={s.addrIcon}>📍</Text>
              <View style={{ flex: 1 }}>
                <View style={s.addrTop}>
                  <Text style={s.addrLabel}>Home</Text>
                  <View style={s.activeBadge}><Text style={s.activeBadgeTxt}>Default</Text></View>
                </View>
                <Text style={s.addrText}>{profile.area}, Bengaluru</Text>
              </View>
          ...
          <View style={s.sec}>
            ...
            <View style={s.menuCard}>
              {[
                ...
                <TouchableOpacity key={i} style={[s.menuRow, i < 1 && s.menuRowBorder]} activeOpacity={0.7}>
                  <Text style={s.menuIcon}>{m.icon}</Text>
                  <View style={{ flex: 1 }}>
          ...
          <View style={[s.sec, { paddingBottom: 4 }]}>
            ...
            <Text style={s.version}>Sprinkle Water v1.0.0 · {profile.area}, Bengaluru</Text>
          </View>
...
const s = StyleSheet.create({
  ...
  statVal:         { fontFamily: SERIF, fontSize: 18, fontWeight: '800', color: GD, marginBottom: 3 },
